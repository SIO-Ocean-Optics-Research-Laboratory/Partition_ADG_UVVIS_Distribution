...
%the ultraviolet (UV) and visible (VIS) spectral region covering the light
%wavelength range from 350 to 700 nm.
%
%%Reference:
%
...
%   optimal solutions).
%
%Required ancillary data file: UVVIS_lib.mat
%   UVVIS_lib.mat: Ultraviolet(UV)-Visible(VIS) spectral shape function
%   library in the spectral range of 350-700 nm required to be in MATLAB
%   path for model to operate. Derived from joint probability distributions
%   of spectral steepness parameters from the development dataset described
%   in the Reference Manuscript.
%
%Outputs: lambda_out, adopt, agopt
%   lambda_out [351-by-1 double]: Light wavelengths [nm] corresponding to
%   output values of spectral absorption coefficients ad and ag from the
%   partitioning model.
%
%   adopt [351-by-(1+length(PT)) double]: Matrix containing the optimal
...
%
%   agopt [351-by-(1+length(PT)) double]: Matrix containing the optimal
%   solutions for spectral values of ag [m^-1] determined as the median
%   value of all feasible solutions at each wavelength (1st column) and
%   additional desired percentile solutions from the pool of feasible
%   solutions (remaining output column(s)).
%
%Version history: 2023-11-09: Partition_ADG_UVVIS.m, revised ADG
%partitioning model and Matlab version, M. D. Kehrli, D. Stramski, R. A.
%Reynolds, I. D. Joshi.
%
...
%nm
idx_dg = find(~isnan(adg) & 350 <= lambda_adg & lambda_adg <= 700);
